...
#[test]
fn test_buffer_boundary_3() -> Result<(), Box<dyn std::error::Error>> {
    let mut cmd = assert_cmd::cargo::cargo_bin_cmd!("binarygrep");
    cmd.arg("--no-ascii")
        .arg("c7ccb128")
        .arg("tests/testdata_10485760");
    let output = "00400000: c7ccb128\n";
    cmd.assert().success().stdout(output);
    cmd.arg("--extended");
    cmd.assert().success().stdout(output);
    Ok(())
